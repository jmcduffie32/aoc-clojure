...
(defn handle-left [path distance]
  (let [[x0 y0] (first path)
        x1 (dec x0)]
    (if (= distance 0)
      path
      (handle-left (cons [x1 y0] path) (dec distance)))))

(defn handle-right [path distance]
  (let [[x0 y0] (first path)
        x1 (dec x0)]
    (if (= distance 0)
      path
...
(defn handle-down [path distance]
  (let [[x0 y0] (first path)
        x1 (dec x0)]
    (if (= distance 0)
      path
...
(defn generate-path [instruction path]
  (let [[direction distance] instruction]
    (cond
      (= direction \R) (handle-right path distance)
      (= direction \L) (handle-left path distance)
      (= direction \U) (handle-up path distance)
      (= direction \D) (handle-down path distance))))
